...
void SimplifiedEncoder::create_vars_for_makespan(int makespan) {
	...
	int created_vars_makespan = -1;		// Non incremental

	while (created_vars_makespan < makespan) {
		++created_vars_makespan;
		...
		for (int j = 0; j < _instance.n_vertices(); ++j) {
			for (unsigned k = 0; k < _instance.n_agents(); ++k) {
				...
				if (_verbose > 1)
					std::cout << "var x for agent " << k
							  << ", vertex " << j
							  << ", and timestep " << created_vars_makespan
							  << " has id " << make_xvar_id(k, j, created_vars_makespan);

				while (make_xvar_id(k, j, created_vars_makespan) >= _solver->nVars())
					_solver->newVar();
			...
			if (_verbose > 1)
				std::cout << "var epsilon for vertex " << j
						  << ", and timestep " << created_vars_makespan
						  << " has id " << make_evar_id(j, created_vars_makespan) << std::endl;

			while (make_evar_id(j, created_vars_makespan) >= _solver->nVars())
				_solver->newVar();
		}
	}
}
...
void SimplifiedEncoder::create_clauses_for_makespan(int makespan) {
	...
	int created_clauses_makespan = 0;	// Non incremental
	...
	while (created_clauses_makespan < makespan) {
		int i = ++created_clauses_makespan;
