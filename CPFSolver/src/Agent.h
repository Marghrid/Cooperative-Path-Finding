...
public:
    explicit Agent(int aid)
            : _id(aid), _initial_position(static_cast<Vertex>(-1)), _goal_position(static_cast<Vertex>(-1)) {}

    Agent(int aid, Vertex initial_pos, Vertex goal_pos)
            : _id(aid), _initial_position(initial_pos), _goal_position(goal_pos) {}

    Agent(int aid, Vertex initial_pos) : Agent(aid, initial_pos, static_cast<Vertex>(-1)) {}
