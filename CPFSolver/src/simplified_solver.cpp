...
Glucose::Var Simplified_solver::make_xvar_id(int agent_id, int vertex_id, int timestep, int eta) {
	return timestep * _instance.n_vertices() * (_instance.n_agents() + 1)
		+ vertex_id * (_instance.n_agents() + 1)
		+ agent_id;
}

Glucose::Var Simplified_solver::make_evar_id(int vertex_id, int timestep, int eta) {
	return timestep * _instance.n_vertices() * (_instance.n_agents() + 1)
		+ vertex_id * (_instance.n_agents() + 1)
		+ _instance.n_agents();
}
...
bool Simplified_solver::solve_for_makespan(Glucose::SimpSolver solver, int eta) {
		...
		for(int j = 0; j < n; ++j) {
			for(int k = 0; k < mu; ++k) {
				std::cout << "var x for agent " << k
					<< ", vertex " << j
					<< ", and timestep " << i
					<< " has id " << make_xvar_id(k, j, i, eta) << std::endl;
					
					while (make_xvar_id(k, j, i, eta) >= solver.nVars()) solver.newVar();
			}

			std::cout << "var epsilon for vertex " << j
				<< ", and timestep " << i
				<< " has id " << make_evar_id(j, i, eta) << std::endl;
				
			while (make_evar_id(j, i, eta) >= solver.nVars()) solver.newVar();
		}
	}
	...
	for(int i = 0; i < eta; ++i) {
		for(int j = 0; j < n; ++j){
			for(int h = 0; h < mu; ++h) {
				for(int k = 0; k < h; ++k) {
					Glucose::Lit l1 = Glucose::mkLit(make_xvar_id(k, j, i, eta), true);
					Glucose::Lit l2 = Glucose::mkLit(make_xvar_id(h, j, i, eta), true);
	...
	for(int i = 0; i < eta-1; ++i) {
		for(int j = 0; j < n; ++j){
			for(int k = 0; k < mu; ++k) {
				...
				lit_vec.push( Glucose::mkLit(make_xvar_id(k, j, i, eta),   true) );
				lit_vec.push( Glucose::mkLit(make_xvar_id(k, j, i+1, eta), false) );

				for(auto &v: _instance.get_neighbours(j)) {
					lit_vec.push( Glucose::mkLit(make_xvar_id(k, v.id(), i+1, eta)) );
				}
	...
	for(int i = 0; i < eta - 1; ++i) {
		for(int k = 0; k < mu; ++k) {
			for(auto &e: _instance.bidirectional_edges()) {
				Glucose::vec<Glucose::Lit> lit_vec;
				lit_vec.push( Glucose::mkLit(make_xvar_id(k, e.start().id(), i, eta), true)  );
				lit_vec.push( Glucose::mkLit(make_xvar_id(k, e.end().id(), i+1, eta), true)  );
				lit_vec.push( Glucose::mkLit(make_evar_id(e.end().id(),      i, eta), false) );
				lit_vec.push( Glucose::mkLit(make_evar_id(e.start().id(),  i+1, eta), false) );
				solver.addClause(lit_vec);
			}
		}
	}
	...
	for(int i = 0; i < eta; ++i) {
		for(int j = 0; j < n; ++j) {
			Glucose::Lit l1 = Glucose::mkLit(make_evar_id(j, i, eta), true);
			for(int k = 0; k < _instance.n_agents(); ++k) {
				solver.addClause(l1, Glucose::mkLit(make_xvar_id(k, j, i, eta), true));
			}
	...
	for(int k = 0; k < mu; ++k) {
		for(int j = 0; j < n; ++j) {
			if(j == _instance.agent(k).initial_position().id()) {
				solver.addClause(Glucose::mkLit(make_xvar_id(k, j, 0, eta)));
			} else {
				solver.addClause(Glucose::mkLit(make_xvar_id(k, j, 0, eta), true));
			}

			if(j == _instance.agent(k).goal_position().id()) {
				solver.addClause(Glucose::mkLit(make_xvar_id(k, j, eta-1, eta) ) );
			} else {
				solver.addClause(Glucose::mkLit(make_xvar_id(k, j, eta-1, eta), true) );
			}
		}
	}
	...
	for(int i = 0; i < eta; ++i) {
		...
		for(int k = 0; k < mu; ++k) {
			for(int j = 0; j < n; ++j) {
				if(solver.modelValue(make_xvar_id(k, j, i, eta)) == l_True)
					std::cout << "\tAgent " << k
						<< " is on vertex " << j
						<< std::endl;
			}
