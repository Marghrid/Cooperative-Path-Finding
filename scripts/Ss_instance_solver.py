...
for filename in instances:
	...
	aux += " --output-file=../Ss_solutions/" + instance + "_unsat-sat.out"
	makespan_limit = get_makespan_limit(instance)
	aux += " --makespan-limit=" + str(makespan_limit)
	aux += " --strategy=linear-up"
	aux += " --encoding=simplicial"
	aux += " > ../Ss_solver_files/" + instance + "_unsat-sat.txt 2>&1"

	print ("../reLOC-0.13-odaiba_037/src/solver_reLOC" + aux)
	system("../reLOC-0.13-odaiba_037/src/solver_reLOC" + aux)

	aux  = " --input-file=../instances/"  + filename
	aux += " --output-file=../Ss_solutions/" + instance + "_binary.out"
	makespan_limit = get_makespan_limit(instance)
	aux += " --makespan-limit=" + str(makespan_limit)
	aux += " --strategy=binary"
	aux += " --encoding=simplicial"
	aux += " > ../Ss_solver_files/" + instance + "_binary.txt 2>&1"
